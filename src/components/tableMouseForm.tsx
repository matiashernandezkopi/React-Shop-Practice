...
export const MouseForm: React.FC<MouseFormProps> = ({ isOpen, onSubmit, onClose, type, actualCellValue }) => {
    const [formData, setFormData] = useState<string>(actualCellValue)

    useEffect(() => {
        setFormData(actualCellValue.toString())
    }, [isOpen, actualCellValue])
    ...
    useEffect(() => {
        if (type === "amount") {
            const numValue = convertToNumber(formData)
            setFormData(numValue.toString())
        }
    }, [type, formData])
